'''
This part is used to train the semi-supervised speaker model with flexmatch and evaluate the performances
'''
...
class ECAPAModel(nn.Module):
	...
	def train_network(self, putoff, epoch, loader1, loader2):
		...
		# Initialize the parameters of the CPL
		selected_label = torch.ones((1092009,), dtype=torch.long,)*-1
		selected_label = selected_label.cuda()
		classwise_acc = torch.zeros((5994,)).cuda()

		## Update the learning rate based on the current epcoh
		self.scheduler.step(epoch - 1)
		tstart = time.time()
		index, top1, loss, loss1, quality, quantity, num, indexulb = 0, 0, 0, 0, 0, 0, 0, 0
		...
		#for (idx, data, labels), (x_ulb_idx, x_ulb_w, x_ulb_s, y_ulb) in zip(loader1, loader2):
			...
			if max(pseudo_counter.values()) < 1092009:
				for i in range(5994):
					classwise_acc[i] = pseudo_counter[i]/max(pseudo_counter.values())
			labels = torch.LongTensor(labels.numpy()).cuda()
			y_ulb = torch.LongTensor(y_ulb.numpy()).cuda()
			...
			nloss1, prec = self.speaker_loss.forward(logits_lb.cuda(), mask=1, label=labels)
			...
			select = max_probsulb_w.ge(putoff).long()
			mask = max_probsulb_w.ge(putoff*(classwise_acc[max_idxulb_w]/(2.-classwise_acc[max_idxulb_w]))).long()
			if x_ulb_idx[select == 1].nelement() != 0:
				selected_label[x_ulb_idx[select == 1]] = max_idxulb_w[select == 1].long()
