...
class model(nn.Module):
    ...
    def train_network(self, loader, epoch):
        ...
        for counter, data in enumerate(loader, start = 1):     
            ...
            feat = []
            for inp in data:
                embedding,_=self.Network.forward(torch.FloatTensor(inp).cuda())
                feat.append(embedding) # Feed the segments to get the speaker embeddings
    ...
    def evaluate_network(self, val_list, val_path, **kwargs):
        ...
        for idx, file in tqdm.tqdm(enumerate(setfiles), total = len(setfiles)):
            ...
            with torch.no_grad():
                ref_feat,_ = self.Network.forward(feat)
            feats[file]     = ref_feat.detach().cpu() # Extract features for each data, get the feature dict
        scores, labels  = [], []
        for line in open(val_list).read().splitlines():
            data = line.split()
