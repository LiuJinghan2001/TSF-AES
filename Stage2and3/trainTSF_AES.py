'''
This is the main code of the Stage2 and Stage3 of TSF-AES, to define the parameters and build the construction
'''

import argparse, glob, warnings, time, sys, yaml
from tools import *
from dataLoader1 import train_loader
from TSF_AES import ECAPAModel
...
def main_worker(args):
	...
	Lb_trainloaderS2 = torch.utils.data.DataLoader(lb_trainloader, batch_size=args.batch_size_s2, shuffle=True,
												   num_workers=args.n_cpu, drop_last=True)
	Lb_trainloaderS3 = torch.utils.data.DataLoader(lb_trainloader, batch_size=args.batch_size_s3, shuffle=True,
											  num_workers=args.n_cpu, drop_last=True)
	Ulb_trainloader = torch.utils.data.DataLoader(ulb_trainloader, batch_size=args.batch_size_s3 * args.uratio, shuffle=True,
												 num_workers=args.n_cpu, drop_last=True)
	...
	while (1):
		...
		loss, lr, acc, minp, quality, quantity = s.train_network(putoff=args.putoff, epoch=epoch, loader1=Lb_trainloaderS3, loader2=Ulb_trainloader, loader3=Lb_trainloaderS2)
		...
		if epoch % args.test_step == 0:
			...
			score_file.write("%d epoch, LR %f, LOSS %f, ACC %2.2f%%, EER %2.2f%%, bestEER %2.2f%%, minDCF %2.2f%%, putoff :%.6f, quality :%.4f, quantity :%.4f\n" % (
			epoch, lr, loss, acc, EERs[-1], min(EERs), minDCF, args.putoff, quality, quantity))
			score_file.flush()

			##  check whether the model performs best
			check = s.check(EERs, EERm)

			if check == True:
				##  get init threshold for Stage 3
				initp = s.getinitp(loader=Lb_trainloaderS2)
				#print("initp = %.6f " % initp)
				##  implement AES, update confidence threshold
				P.append(initp)
				args.putoff = P[0] - (P[0] - minp) * epochS3 / args.max_epochS3
				print("change %.6f" % args.putoff)
				stage = 3
				EERm = []

			if stage == 3:
				epochS3 += 1
		if epochS3 >= args.max_epochS3 + 5:
			quit()
		epoch += 1
...
if __name__ == "__main__":
	parser = argparse.ArgumentParser(description="Stage 2 and 3, semi-supervised speaker recognition with TSF-AES.")
	## Training Settings
	parser.add_argument('--num_frames', type=int, default=200,
						help='Duration of the input segments, eg: 200 for 2 second')
	parser.add_argument('--max_epochS3', type=int, default=30, help='Maximum number of epoch only in S3')
	parser.add_argument('--batch_size_s2', type=int, default=200, help='Batch size of stage2')
	parser.add_argument('--batch_size_s3', type=int, default=150, help='Batch size of stage3')
	...
	parser.add_argument("--putoff", type=float, default=1, help='Confidence threshold')
	...
	parser.add_argument('--initial_model', type=str, default="/home/ljh/home/ljh/SSL/TSF-AES/Stage1/exps/contrast/S1_7.02.model",
	...
	parser.add_argument('--lb_samples_per_class', type=float, default=10, help='Number of speaker labels per class')
	## Command
